...
public class SkuServiceImpl implements SkuService {
    ...
    public List<Sku> findList(Map<String, Object> searchMap){
        ...
        if (spuId == null || spuId.length() <= 0){
            example.createCriteria().andEqualTo("status",searchMap.get("status"));
            return skuMapper.selectByExample(example);
        }
        example.createCriteria().andEqualTo("spuId",searchMap.get("spuId")).andEqualTo("status",searchMap.get("status"));
        return skuMapper.selectByExample(example);
    }
    ...
    public void updateCount(String username) {
        ...
        for (OrderItem orderItem : orderItemList) {
            ...
            //影响行数
            int count = skuMapper.updateCount(skuId,num);
            if (count <=  0){
    ...
    @Override
    public void addNum(String skuId, Integer num) {
